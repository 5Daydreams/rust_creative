...
fn model(_app: &App) -> Model {
    ...
    for _ in 0..model.boid_count {
        ...
        let x_range_pos: Range<f32> = -(model.window_size[0] as f32)..(model.window_size[0] as f32);
        let y_range_pos: Range<f32> = -(model.window_size[1] as f32)..(model.window_size[1] as f32);
        ...
        let perception_radius: f32 = 355.;
        let max_speed: f32 = 5.5;
        
		let steering_factor: f32 = 0.003;
        let cohesion_factor: f32 = 0.005;
        let separation_factor: f32 = 0.003;
...
fn view(_app: &App, model: &Model, frame: Frame) {
    ...
    draw.background().color(Srgb::<f32>::new(0.05, 0.1, 0.15));
